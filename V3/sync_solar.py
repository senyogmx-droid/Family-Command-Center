...
if config.get("enphase_gateway_ip"):
    ENVOY_IP = config["enphase_gateway_ip"]
elif config.get("envoy_ip"):
    ENVOY_IP = config["envoy_ip"]

# Try loading dynamic token from a local text file to allow headless updates, falling back to config or default
ENVOY_TOKEN = config.get("enphase_token", "")
...
if os.path.exists(token_file):
    try:
        ...
        print(f"[!] Error reading solar_token.txt: {token_err}. Using config/fallback token.")
...
def fetch_local_solar_data():
    ...
    try:
        ...
        solar_json_path = os.path.join(os.path.dirname(os.path.abspath(__file__)), 'solar.json')
        with open(solar_json_path, 'w', encoding='utf-8') as f:
